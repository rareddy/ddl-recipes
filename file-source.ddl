...
/subsystem=teiid:db-store(vdb-name=foo, vdb-version=1, ddl="create server \"text-connector\" type 'file' foreign data wrapper file options(ParentDirectory '/home/rareddy/testing/vdbs/marketdata')")
/subsystem=teiid:db-store(vdb-name=foo, vdb-version=1, ddl="create schema MarketData server \"text-connector\"")
/subsystem=teiid:db-store(vdb-name=foo, vdb-version=1, schema=MarketData, ddl="import foreign schema public from server \"text-connector\" into MarketData")
